...
%       % Crear una cuadrícula de ocupación
%       map = robotics.OccupancyGrid(eye(10));
% 
%       % Establecer la ocupación en la coordenada del mundo (5, 5)
%       setOccupancy(map, [5 5], 0.5);
% 
%       % Obtener los puntos de colisión
%       collisionPts = rayIntersection(map, [0,0,0], [-pi/2, pi/2], 15);
% 
%       % Visualizar los puntos de colisión
%       show(map);
%       hold('on');
...
%         Voy desde pose intermedia hasta la puerta del labo
        path = planning_framework(intermedio1,intermedio2 );
        path = planning_framework(intermedio2,objetivo1 );
